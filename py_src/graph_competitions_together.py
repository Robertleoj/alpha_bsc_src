...
def plot_competition_results(csv: pd.DataFrame, title):
    ...
    sns.lineplot(data=csv, x='playouts2', y='win_rate', hue='run_name', markers=True, dashes=False)
...
def plot_competition_results_conf(csv: pd.DataFrame, conf, title):
    ...
    csv['se'] = np.sqrt((csv['win_rate'] * (1 - csv['win_rate'])) / n)
    ...
    for run_name, color in zip(csv['run_name'].unique(), sns.color_palette()):
        ...
        plt.fill_between(temp_df['playouts2'], temp_df['lower'], temp_df['upper'], color=color, alpha=0.2)
...
def main():
    ...
    if len(argv) > 1:
        ...
        title = f'Breakthrough: Win Rates vs Playouts'
